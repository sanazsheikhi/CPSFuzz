...
crash_hist = array.array('i', (0 for i in range(0,8)))
...
class LaneSwitcher:
    def __init__(self, lanes, num_lanes, current_lane, proximity_distance=2.5):
        # Assumptions for lanes vector layout: each lane occupies 3 columns (x, y, vel), 
        # last three columns is the raceline, previous are lanes from inner to outer, 
        # corresponding to increase in indexing
...
def run(arr): 
    ...
    input_vector_scale_speed = arr

    input_index = 0
    #cov = coverage.Coverage()
    path = "/home/sanaz/Documents/Research/f1tenth_gym/gym/f110_gym/envs/" 
    #cov_sbb = tuple() #tmp for compiler error
    cov_sbb = set()
    #io = 0 # test
    while not done:
        ...
        dist = math.sqrt((x_e_n - x_o_n)**2 + (y_e_n - y_o_n)**2) 
        if dist > 10:
            return [0,frames, x_e_n, y_e_n]


        if (done == 1):
            crash = 0             

            return [done, frames, x_e_n, y_e_n] 
